...
{
    s_pLogger = new (std::nothrow) imif::common::logging(false, std::string("imif_tcp_sender"));
    ...
    // log version
    LOG(INFO) << "Running " << std::string("imif_tcp_sender") << " Version " << IMIF_VERSION << " Build date " << IMIF_BUILD_DATE
              << std::endl
              << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    imif::sender::tcp_sender_thread tcp_sender_thread(broker_path, s_pLogger);

    if (!tcp_sender_thread.start()) {
        LOG(ERROR) << "Faild to start thread! ";
        g_running = false;
    }

    while (g_running) {

        if (!tcp_sender_thread.is_running()) {
