...
{
    s_pLogger = new (std::nothrow) imif::common::logging(false, std::string(IMIF_MSTREAM));
    ...
    // log version
    LOG(INFO) << "Running " << std::string(IMIF_MSTREAM) << " Version " << IMIF_VERSION << " Build date " << IMIF_BUILD_DATE
              << std::endl
              << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    imif::mstream::MstThread mstream_thread(broker_path, s_pLogger);

    if (!mstream_thread.start()) {
