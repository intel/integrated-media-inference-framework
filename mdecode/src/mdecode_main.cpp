...
{
    s_pLogger = new (std::nothrow) imif::common::logging(false, std::string(IMIF_MDECODE));
    ...
    // log version
    LOG(INFO) << "Running " << std::string(IMIF_MDECODE) << " Version " << IMIF_VERSION << " Build date " << IMIF_BUILD_DATE
              << std::endl
              << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    imif::mdecode::MDecodeThread mdecode_thread(broker_path, s_pLogger);

    if (!mdecode_thread.start()) {
